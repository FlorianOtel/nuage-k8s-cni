...
// XXX - this is only needed because it has to be a named type
type nscache map[string]namespace // Key: K8S Namespace Name
...
var (
	...
	Namespaces struct {
		sync.Mutex
		nscache // Key: K8S Namespace Name
	}
...
func InitClient(conf *config.AgentConfig) error {
	...
	////
	Namespaces.nscache = make(map[string]namespace)
