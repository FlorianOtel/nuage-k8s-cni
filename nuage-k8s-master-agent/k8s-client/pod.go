...
func PodCreated(pod *apiv1.Pod) error {
	...
	if podNsZone, exists = Namespaces.nscache[pod.ObjectMeta.Namespace]; !exists {
		...
		for {
			...
			case <-tickChan:
				if podNsZone, found = Namespaces.nscache[pod.ObjectMeta.Namespace]; !found {
					continue
				}
			}
			break
	...
	if cifaddr == nil { // We could not get any lease from any non-custom subnet above
		...
		for newprefix, newcidr := range vsdclient.FreeCIDRs {
			if newcidr != nil { // True for any valid entry in the map
				...
				Namespaces.Lock()
				Namespaces.nscache[pod.ObjectMeta.Namespace] = podNsZone
				Namespaces.Unlock()
...
func PodDeleted(pod *apiv1.Pod) error {
	...
	// found := false
	for _, subnet := range Namespaces.nscache[pod.ObjectMeta.Namespace].Subnets {
